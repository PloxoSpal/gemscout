...
SMSC_LOGIN = settings.SMS_LOGIN					# логин клиента
SMSC_PASSWORD = settings.SMS_PASSWORD				# пароль клиента. Если передан пустой логин, то SMSC_PASSWORD используется, как API ключ, вместо логина и пароля
...
class SMSC(object):
	...
	def _smsc_send_cmd(self, cmd, arg=""):
		...
		_url = url
		arg = ifs(SMSC_LOGIN, "login=" + quote(SMSC_LOGIN) + "&psw=", "apikey=") + quote(SMSC_PASSWORD) + "&fmt=1&charset=" + SMSC_CHARSET + "&" + arg
